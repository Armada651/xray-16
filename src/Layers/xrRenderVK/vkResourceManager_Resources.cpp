...
#include "Layers/xrRenderVK/vkBufferUtils.h"
...
//--------------------------------------------------------------------------------------------------------------
vkConstantBuffer* CResourceManager::_CreateConstantBuffer(const glslang::TType* pTable, u32 size)
{
    VERIFY(pTable);
    vkConstantBuffer* pTempBuffer = new vkConstantBuffer(pTable, size);

    for (u32 it = 0; it < v_constant_buffer.size(); it++)
    {
        vkConstantBuffer* buf = v_constant_buffer[it];
...
//--------------------------------------------------------------------------------------------------------------
void CResourceManager::_DeleteConstantBuffer(const vkConstantBuffer* pBuffer)
{
