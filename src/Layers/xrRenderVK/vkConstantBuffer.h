#ifndef vkConstantBuffer_included
#define vkConstantBuffer_included
#pragma once

struct R_constant;
struct R_constant_load;

class vkConstantBuffer : public xr_resource_named
{
public:
    vkConstantBuffer(const glslang::TType* pTable, u32 size);
    ~vkConstantBuffer();

    bool Similar(vkConstantBuffer& _in);
...
private:
    ...
    VkBufferUsageFlags m_eBufferType;

    //	Buffer data description
    u32 m_uiMembersCRC;
    glslang::TType* m_Table;
    ...
    vkConstantBuffer(const vkConstantBuffer&);
    vkConstantBuffer& operator=(vkConstantBuffer&);
};

typedef resptr_core<vkConstantBuffer, resptr_base<vkConstantBuffer>> ref_cbuffer;

#endif //	vkConstantBuffer_included
