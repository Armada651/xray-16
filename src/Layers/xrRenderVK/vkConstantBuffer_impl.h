#ifndef vkConstantBuffer_impl_included
#define vkConstantBuffer_impl_included
#pragma once

IC Fvector4* vkConstantBuffer::Access(u16 offset)
{
    //	TODO: vk: Implement code which will check if set actually changes code.
    m_bChanged = true;
...
IC void vkConstantBuffer::set(R_constant* C, R_constant_load& L, const Fmatrix& A)
{
...
IC void vkConstantBuffer::set(R_constant* C, R_constant_load& L, const Fvector4& A)
{
    VERIFY(RC_float == C->type);
...
IC void vkConstantBuffer::set(R_constant* C, R_constant_load& L, float A)
{
    VERIFY(RC_float == C->type);
    VERIFY(RC_1x1 == L.cls);
    float* it = (float*)Access(L.index);
...
IC void vkConstantBuffer::set(R_constant* C, R_constant_load& L, int A)
{
...
IC void vkConstantBuffer::seta(R_constant* C, R_constant_load& L, u32 e, const Fmatrix& A)
{
    //	TEST
    // return;
    VERIFY(RC_float == C->type);
...
IC void vkConstantBuffer::seta(R_constant* C, R_constant_load& L, u32 e, const Fvector4& A)
{
    //	TEST
    // return;
    VERIFY(RC_float == C->type);
...
IC void* vkConstantBuffer::AccessDirect(R_constant_load& L, u32 DataSize)
{
    //	Check buffer size in client code: don't know if actual data will cross
    //	buffer boundaries.
...
#endif //	vkConstantBuffer_impl_included
