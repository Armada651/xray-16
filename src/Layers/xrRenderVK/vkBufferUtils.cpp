...
DXGI_FORMAT	ConvertVertexFormat(D3DDECLTYPE dx9FMT)
{
	...
	for (int i=0; i<arrayLength; ++i)
	{
		if (VertexFormatList[i].m_dx9FMT==dx9FMT)
			return VertexFormatList[i].m_vkFMT;
	}

	VERIFY(!"ConvertVertexFormat didn't find appropriate vk vertex format!");
...
LPCSTR	ConvertSemantic(D3DDECLUSAGE Semantic)
{
	...
	for (int i=0; i<arrayLength; ++i)
	{
		if (VertexSemanticList[i].m_dx9Semantic==Semantic)
			return VertexSemanticList[i].m_vkSemantic;
	}

	VERIFY(!"ConvertSemantic didn't find appropriate vk input semantic!");
	return 0;
}
